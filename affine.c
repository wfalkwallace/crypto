...
//   
//   A set of methods to encrypt/decrypt according to a simple affine cipher;
//   currently only works as expected for ascii (would work for others, but the
//   beta would be unexpected and decryption would require reusing these
//   methods, but should work fine - would require an additional beta of 65, 
...
//encrypt and print
int pencrypt(const char *plain, int alpha, int beta)
{
	...
	for(i = j = 0; i < length; i++, j++) 
	{
		//encrypt character
		if(!isspace(cipher[i]))
			cipher[j] = ((toupper(cipher[i]) - 65 + beta) * alpha) % 26 + 65;
...
//decrypt and print
int pdecrypt(const char *cipher, int alpha, int beta)
{
	//get string length and make non-const string copy
	...
	for(i = j = 0; i < length; i++, j++) 
	{
		//encrypt character
		if(!isspace(plain[i]))
			plain[j] = ((plain[i] - 65 - beta) / alpha) % 26 + 65;
...
//file encrypt
FILE *fencrypt(char *input, char *output, int alpha, int beta)
{
	...
	while((p = getc(plain)) != EOF)
	{
		...
		if(!isspace(p))
			//make it uppercase, get rid of the ascii beta
			putc(((toupper(p) - 65 + beta) * alpha)% 26 + 65, cipher);
...
//file decrypt
FILE *fdecrypt(char *input, char *output, int alpha, int beta)
{
	...
	while((c = getc(cipher)) != EOF)
	{
		...
		//spaces shouldn't matter though; get rid of the ascii beta
		putc(((toupper(c) - 65 - beta) / alpha)% 26 + 65, plain);
	}
	//close 'em up
	fclose(plain);
	fclose(cipher);
...
int main(int argc, char *argv[])
{
	...
	if (argc != 5) {
		fprintf(stderr, "Usage: %s <e/d> <input-file> <output-file> <beta>\n", argv[0]);
		exit(1);
	}
